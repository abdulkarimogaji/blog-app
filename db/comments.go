...
func (d *DBStruct) GetComments(ctx context.Context, filters GetCommentsFilters, params PaginationParams) ([]Comment, int, error) {
	...
	var total int
	row := d.DB.QueryRowContext(ctx, fmt.Sprintf("SELECT COUNT(id) as total FROM comments WHERE %s", where))
	err = row.Scan(&total)
	if err != nil {
		return []Comment{}, 0, err
	}
...
func (d *DBStruct) CreateComment(ctx context.Context, body CreateCommentRequest) (Comment, error) {
	// check if author exists
	var author User
	row := d.DB.QueryRowContext(ctx, "SELECT id, first_name, last_name, email, created_at, updated_at  from users WHERE id = ?;", body.UserId)
	...
	var blog Blog
	row = d.DB.QueryRowContext(ctx, "SELECT id from blogs WHERE id = ?;", body.BlogId)
	err = row.Scan(&blog.Id)
	if err != nil {
		return Comment{}, err
	}

	stmt, err := d.DB.PrepareContext(ctx, "INSERT INTO comments (user_id, blog_id, message, posted_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?);")
	if err != nil {
		return Comment{}, err
	}
	...
	result, err := stmt.ExecContext(ctx, body.UserId, body.BlogId, body.Message, postedAt, createdAt, updatedAt)
