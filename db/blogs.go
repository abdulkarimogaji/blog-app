...
func (d *DBStruct) CreateBlog(ctx context.Context, body CreateBlogRequest) (Blog, error) {
	// check if author exists
	var author User
	row := d.DB.QueryRowContext(ctx, "SELECT id, first_name, last_name, email, created_at, updated_at  from users WHERE id = ?;", body.AuthorId)
	err := row.Scan(&author.Id, &author.FirstName, &author.LastName, &author.Email, &author.CreatedAt, &author.UpdatedAt)
	if err != nil {
		return Blog{}, err
	}

	stmt, err := d.DB.PrepareContext(ctx, "INSERT INTO blogs (author_id, title, slug, excerpt, thumbnail, body, posted_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);")
	if err != nil {
		return Blog{}, err
	}
	...
	result, err := stmt.ExecContext(ctx, body.AuthorId, body.Title, slug, body.Excerpt, body.Thumbnail, body.Body, postedAt, createdAt, updatedAt)
...
func (d *DBStruct) GetBlogBySlug(ctx context.Context, slug string) (Blog, error) {
	var blog Blog
	row := d.DB.QueryRowContext(ctx, "SELECT blogs.*, users.id, first_name, last_name, email, photo, socials from blogs LEFT JOIN users ON users.id = author_id LEFT JOIN profile ON profile.user_id = author_id WHERE slug = ?;", slug)
...
func (d *DBStruct) GetBlogById(ctx context.Context, id int) (Blog, error) {
	var blog Blog
	row := d.DB.QueryRowContext(ctx, "SELECT blogs.*, users.id, first_name, last_name, email, photo, socials from blogs LEFT JOIN users ON users.id = author_id LEFT JOIN profile ON profile.user_id = author_id WHERE blogs.id = ?;", id)
	err := row.Scan(&blog.Id, &blog.AuthorId, &blog.Title, &blog.Slug, &blog.Excerpt, &blog.Thumbnail, &blog.Body, &blog.PostedAt, &blog.CreatedAt, &blog.UpdatedAt, &blog.Author.Id, &blog.Author.FirstName, &blog.Author.LastName, &blog.Author.Email, &blog.Author.Photo, &blog.Author.Socials)
	if err != nil {
		return Blog{}, err
	}
...
func (d *DBStruct) GetBlogs(ctx context.Context, filters GetBlogsFilters, params PaginationParams) ([]Blog, int, error) {
	...
	var total int
	row := d.DB.QueryRowContext(ctx, fmt.Sprintf("SELECT COUNT(blogs.id) as total FROM blogs LEFT JOIN users ON users.id = author_id WHERE %s", where))
	err = row.Scan(&total)
	if err != nil {
		return []Blog{}, 0, err
	}
